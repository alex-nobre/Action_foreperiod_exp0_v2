...
data <- data %>%
  mutate(oneBackFPDiff = case_when(numOneBackFPDiff>0 ~ "1",
                                   numOneBackFPDiff<0 ~ "-1",
                                   numOneBackFPDiff==0 ~ "0")
         ) %>%
  mutate(oneBackFPDiff = as.factor(oneBackFPDiff)) %>%
  mutate(oneBackFPDiff=forcats::fct_relevel(oneBackFPDiff, levels=c("-1", "0", "1")))
...
data$logOneBackFP <- log2(data$numOneBackFP)
...
summaryData <- data %>%
  group_by(ID,foreperiod,condition,
           orientation,prevOri,
           oneBackFP,twoBackFP,oneBackEffect,numOneBackFPDiff,#oneBackFPDiff,

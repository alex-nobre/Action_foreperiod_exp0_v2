...
source('G:/My Drive/Post-doc/Projetos/Action_foreperiod/Experimento_0_v2/Analysis/Prepare_data_exp0_v2.R')
...
# 
# contrasts(data$oneBackFPDiff) <- contrastCodes
...
#   set_colnames(c('nogoVsShorter', 'longerVsShorter', 'equalVsShorter'))
# 
# contrasts(data2$oneBackFPDiff) <- contrastCodes
...
cor(fitted(fplmm1), data$RT)^2
cor(fitted(invfplmm1), data$invRT)^2
...
# Amount of variance accounted for by the model
cor(fitted(trimfplmm), data2$RT)^2
cor(fitted(triminvfplmm), data2$invRT)^2
...
# Model comparisons by systematically removing random effects components
...
cor(fitted(triminvfplmm), data2$RT)^2
cor(fitted(triminvfplmm2), data2$invRT)^2
cor(fitted(triminvfplmm3), data2$invRT)^2
cor(fitted(triminvfplmm4), data2$invRT)^2

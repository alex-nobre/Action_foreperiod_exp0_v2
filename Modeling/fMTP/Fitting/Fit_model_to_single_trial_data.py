...
Still does not work because of the no-go trials, which have nan RT values
"""
...
#empData = empData.groupby(['ID'], as_index=False)
...
for i, iID in enumerate(IDList):
    ...
    IDDataAction=empDataAction.loc[empDataAction.ID==iID]
    IDDataExternal=empDataExternal.loc[empDataExternal.ID==iID]
    ...
    actionExp = FPexp(FPs = FPs, FPList = IDFPAction, distribution = distr, tr_per_block = 150)
    externalExp = FPexp(FPs = FPs, FPList = IDFPExternal, distribution = distr, tr_per_block = 150)
    ...
    for ik in range(len(kList)):
        for ir in range(len(rList)):
            for ic in range(len(cList)):
                ...
                # External
                sseExternal = op.minimize(sse, startVals, args = (empDataExternal, simExternal), method = 'L-BFGS-B')
                rmseExternal = op.minimize(rmse, startVals, args = (empDataExternal, simExternal), method = 'L-BFGS-B')
                antiCorrExternal = op.minimize(anticorr, startVals, args = (empDataExternal, simExternal), method = 'L-BFGS-B')
                RsseExternal = np.corrcoef((sseExternal.x[0] * simExternal.prep + sseExternal.x[1]), empDataExternal.RT)[0][1]**2
                RrmseExternal = np.corrcoef((rmseExternal.x[0] * simExternal.prep + rmseExternal.x[1]), empDataExternal.RT)[0][1]**2
                RantiCorrExternal = np.corrcoef((antiCorrExternal.x[0] * simExternal.prep + antiCorrExternal.x[1]), empDataExternal.RT)[0][1]**2

...
hazardResultsDF = pd.read_csv('./Modeling/fMTP/Fitting/fittingResults_krc.csv')
...
stCoefsAction = stCoefsAction[['ID', 'k', 'rAction', 'cAction', 'One_minus_corr_Action', 'R2_1-corr_Action']]
stCoefsExternal = hazardResultsDF.loc[hazardResultsDF.groupby('ID').One_minus_corr_External.idxmin()].reset_index()
stCoefsExternal = stCoefsExternal[['k', 'rExternal', 'cExternal', 'One_minus_corr_External',  'R2_1-corr_External']]

# stCoefs = pd.DataFrame(pd.concat([stCoefsAction.rename(columns = {'ID':'ID', 'k':'kAction', 'rAction':'rAction', 'cAction':'cAction', 
...
#                                  axis = 1), columns = ['ID', 'kAction', 'rAction', 'cAction', 'One_minus_corr_Action',
#                                                        'kExternal', 'rExternal', 'cExternal', 'One_minus_corr_External'])
                                                       
stCoefs = pd.DataFrame(pd.concat([stCoefsAction.rename(columns = {'k':'kAction', 'R2_1-corr_Action':'R2_Action'}), 
                                  stCoefsExternal.rename(columns = {'k':'kExternal', 'R2_1-corr_External':'R2_External'})], 
                                 axis = 1), columns = ['ID', 'kAction', 'rAction', 'cAction', 'One_minus_corr_Action', 'R2_Action',
                                                       'kExternal', 'rExternal', 'cExternal', 'One_minus_corr_External', 'R2_External'])
...
for iSub, sub in enumerate(titles):
    ...
    ax[iSub].plot(mean_state_action.FP, mean_state_action.prep,'.--', color = 'blue')
    ax[iSub].plot(empSubAction.foreperiod, empSubAction.RT, '.-', color = 'blue')
    ax[iSub].plot(mean_state_external.FP, mean_state_external.prep,'.--', color = 'orange')
    ax[iSub].plot(empSubExternal.foreperiod, empSubExternal.RT, '.-', color = 'orange')
    
fig.suptitle('ID fits')

figname='./Modeling/fMTP/Fitting/Plots/id_fit_one_minus_corr_krc.png'
...
stCoefsExternal = hazardResultsDF.loc[hazardResultsDF.groupby('ID').One_minus_corr_External.idxmin()].reset_index()
stCoefsExternal = stCoefsExternal[['ID', 'k', 'rExternal', 'cExternal', 'One_minus_corr_External', 'R2_1-corr_External']]
stCoefsExternal['condition'] = pd.Series(['external'] * len(stCoefsExternal))

stCoefsLong = pd.DataFrame(pd.concat([stCoefsAction.rename(columns = {'kAction':'k', 'rAction':'r', 'cAction':'c', 
                                                                   'One_minus_corr_Action':'One_minus_corr',
                                                                   'R2_1-corr_Action':'R2'}), 
                                   stCoefsExternal.rename(columns = {'kExternal':'k', 'rExternal':'r', 'cExternal':'c', 
                                                                     'One_minus_corr_External': 'One_minus_corr',
                                                                     'R2_1-corr_External':'R2'})], 
                                  axis = 0), columns = ['ID', 'condition', 'k', 'r', 'c', 'One_minus_corr', 'R2'])
